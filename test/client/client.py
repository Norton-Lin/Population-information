...
class ClientModel(QObject):
    ...
    def submit(self) -> None:
        ...
        r = requests.post("http://127.0.0.1:8888/Query",
                          json={
                              "type":
                              "Polygon",
                              "coordinates":
                              [[a.x, a.y] for a in self._coordinate_list]
                          })
        ...
        self.coordinate_list_changed.emit()
        if len(r.json().get("response")) == 0:
            return
        data = np.array(r.json().get("response")).transpose((1, 0))
        extent = [
        ...
        ax.title.set_text(
            f"Total population of the area is {r.json().get('total'):.2f}")
        plt.show()
